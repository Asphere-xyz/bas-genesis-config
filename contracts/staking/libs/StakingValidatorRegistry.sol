...
contract StakingValidatorRegistry is StakingStorageLayout, RetryMixin, IStakingValidatorRegistry {
    ...
    function _getValidatorsWithVotingKeys() internal view returns (address[] memory, bytes[] memory) {
        ...
        uint256 k = _STAKING_CONFIG.getActiveValidatorsLength();
        if (k > n) {
            k = n;
        }
    ...
    function forceUnJailValidator(address validatorAddress) external onlyFromGovernor override {
        // make sure validator is in jail
        Validator memory validator = _validatorsMap[validatorAddress];
        require(validator.status == ValidatorStatus.Jail, "bad status");
        // release validator from jail
    ...
    function registerValidator(address validatorAddress, bytes calldata votingKey, uint16 commissionRate, uint256 initialStake) external payable override {
        require(msg.value == initialStake, "bad value");
        // // initial stake amount should be greater than minimum validator staking amount
        require(initialStake >= _STAKING_CONFIG.getMinValidatorStakeAmount(), "too low");
    ...
    function addValidator(address account, bytes calldata votingKey) external onlyFromGovernor override {
        _addValidator(account, votingKey, account, ValidatorStatus.Active, 0, 0, _nextEpoch());
    }

    function removeValidator(address account) external onlyFromGovernor override {
    ...
    function activateValidator(address validatorAddress) external onlyFromGovernor override {
        Validator memory validator = _validatorsMap[validatorAddress];
    ...
    function disableValidator(address validatorAddress) external onlyFromGovernor override {
        Validator memory validator = _validatorsMap[validatorAddress];
    ...
    function slashValidator(address validatorAddress) external onlyFromSlasher override {
        ...
        if (slashesCount == _STAKING_CONFIG.getFelonyThreshold()) {
            validator.jailedBefore = _currentEpoch() + _STAKING_CONFIG.getValidatorJailEpochLength();
