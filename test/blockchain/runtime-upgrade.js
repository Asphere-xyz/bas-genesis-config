...
contract("RuntimeUpgrade", async (accounts) => {
  ...
  it("its possible to upgrade smart contract", async () => {
    ...
    const fakeStakingBytecode = injectorBytecode(systemSmartContracts, StakingWithMethodUnsafe);
    ...
    const newFakeStaking = await StakingWithMethodUnsafe.at(systemSmartContracts[0]);
  ...
  it("upgrade though governance should work", async () => {
    ...
    const systemSmartContracts = await runtimeUpgrade.getSystemContracts();
    const upgradeCall = await runtimeUpgradeContract.methods.upgradeSystemSmartContract(systemSmartContracts[0], injectorBytecode(systemSmartContracts, StakingWithMethodUnsafe), '0x').encodeABI()
